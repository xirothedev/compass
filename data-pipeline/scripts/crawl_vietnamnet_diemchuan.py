...
def parse_school_list(page_html: str):
    """(name, school_code, province, slug_url) from the server-rendered table."""
    rows = []
    for m in re.finditer(
        r'<a[^>]+href="(/giao-duc/score-thi/tra-cuu-score-chuan-cd-dh-\d+/truong/[^"]+)\?keyword=([A-Z0-9]+)"[^>]*>(.*?)</a>\s*</td>\s*<td[^>]*>\s*([A-Z0-9]+)\s*</td>\s*<td[^>]*>(.*?)</td>',
        page_html, re.S):
        url, kw, name_html, code, prov = m.groups()
        name = re.sub(r"\s*\(Xem\)\s*$", "", re.sub(r"<[^>]+>", "", name_html)).strip()
        rows.append({"name": ihtml.unescape(name), "school_code": code,
                     "province": ihtml.unescape(re.sub(r"<[^>]+>", "", prov)).strip(),
                     "url": "https://vietnamnet.vn" + url + f"?keyword={kw}"})
    return rows

def map_method(note: str, sg: str) -> str:
    n = f"{note} {sg}".lower()
...
def main():
    ...
    ap.add_argument("--codes", default="",
                    help="CSV with a school_code column: merged with the crawled list (fills in schools missing from the list)")
    ...
    for year in [y.strip() for y in args.years.split(",") if y.strip()]:
        ...
        page = 0
        while True:
            url = (f"https://vietnamnet.vn/giao-duc/score-thi/tra-cuu-score-chuan-cd-dh-{year}"
                   if page == 0 else
                   f"https://vietnamnet.vn/giao-duc/score-thi/tra-cuu-score-chuan-cd-dh-page{page}")
            html = get(url)
            found = parse_school_list(html)
            if not found: break
            for s in found: schools[s["school_code"]] = s
            page += 1
            if page > 30: break
        ...
        if args.codes:
            ...
            with open(args.codes, encoding="utf-8-sig") as f:
                extra = [r["school_code"].strip() for r in _csv.DictReader(f) if r.get("school_code")]
            added = 0
            for code in extra:
                if code and code not in schools:
                    schools[code] = {"name": "", "school_code": code, "province": "",
                        "url": f"https://vietnamnet.vn/giao-duc/score-thi/tra-cuu-score-chuan-cd-dh-{year}?keyword={code}"}
                    added += 1
        ...
        for i, s in enumerate(items):
            ...
            for sc in model.get("schoolScores") or []:
                try: score = float(str(sc.get("score", "")).replace(",", "."))
                except ValueError: continue
                sg = (sc.get("subjectGroup") or "").strip().upper()
                rows.append({
                    "school_code": code, "school_name": info.get("name") or s["name"],
                    "province": info.get("provinceName") or s["province"],
                    "major_code": (sc.get("code") or "").strip(),
                    "major_name": (sc.get("name") or "").strip(),
                    "combo": sg, "score": score,
                    "method": map_method(sc.get("note") or "", sg),
                    "year": year, "note": (sc.get("note") or "")[:200],
